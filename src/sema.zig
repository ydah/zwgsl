...
const Analyzer = struct {
    ...
    fn warnIfReservedIdentifier(self: *Analyzer, position: ast.Position, name: []const u8) anyerror!void {
        if (std.mem.startsWith(u8, name, "_zwgsl")) {
            try self.diagnostics.appendFmt(
                .warning,
                position.line,
                position.column,
        ...
        if (isWgslReservedIdentifier(name)) {
            try self.diagnostics.appendFmt(
                .warning,
                position.line,
                position.column,
                "identifier '{s}' collides with a WGSL reserved word",
                .{name},
            );
        }
    }
...
fn isWgslReservedIdentifier(name: []const u8) bool {
    ...
    for (reserved) |reserved_name| {
        if (sameName(name, reserved_name)) return true;
    }
    return false;
}
