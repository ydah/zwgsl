...
pub fn build(allocator: std.mem.Allocator, typed: *sema.TypedProgram) anyerror!*hir.Module {
    var builder = Builder{
        .allocator = allocator,
        .typed = typed,
        .lowered_globals = .{},
...
const Builder = struct {
    ...
    lowered_globals: std.ArrayListUnmanaged(hir.Function),
    lowered_specializations: std.StringHashMap(void),
    match_helper_index: usize = 0,

    fn run(self: *Builder) anyerror!*hir.Module {
        const module = try self.allocator.create(hir.Module);
        module.* = .{};
        ...
        module.entry_points = try self.collectEntryPoints();
        module.global_functions = try self.lowered_globals.toOwnedSlice(self.allocator);

        return module;
    }

    fn collectEntryPoints(self: *Builder) anyerror![]const hir.EntryPoint {
        ...
        if (self.typed.vertex_block) |block| {
            try entry_points.append(self.allocator, try self.lowerStage(block, self.typed.vertex_functions));
        }
        if (self.typed.fragment_block) |block| {
            try entry_points.append(self.allocator, try self.lowerStage(block, self.typed.fragment_functions));
        }
        if (self.typed.compute_block) |block| {
            try entry_points.append(self.allocator, try self.lowerStage(block, self.typed.compute_functions));
        }
    ...
    fn collectUniforms(self: *Builder) anyerror![]const hir.Global {
        var items = std.ArrayListUnmanaged(hir.Global){};
    ...
    fn collectStructs(self: *Builder) anyerror![]const hir.StructDecl {
        var items = std.ArrayListUnmanaged(hir.StructDecl){};
        defer items.deinit(self.allocator);

        for (self.typed.program.items) |item| {
            if (item == .struct_def) {
                var fields = std.ArrayListUnmanaged(hir.StructField){};
    ...
    fn lowerAdtConstructors(self: *Builder) anyerror!void {
        for (self.typed.program.items) |item| {
            ...
            for (item.type_def.variants) |variant| {
                ...
                var params = std.ArrayListUnmanaged(hir.Param){};
                defer params.deinit(self.allocator);
                var body = std.ArrayListUnmanaged(hir.Statement){};
    ...
    fn lowerStage(self: *Builder, block: *ast.ShaderBlock, functions: []const *ast.FunctionDef) anyerror!hir.EntryPoint {
        var inputs = std.ArrayListUnmanaged(hir.Global){};
        var outputs = std.ArrayListUnmanaged(hir.Global){};
        var varyings = std.ArrayListUnmanaged(hir.Global){};
        ...
        const stage_io = [3][]const hir.Global{ input_slice, output_slice, varying_slice };
        const lowered_functions = try self.lowerFunctions(functions, block.stage, &stage_io);
        ...
        return .{
            ...
            .main_function_index = findMainFunctionIndex(lowered_functions),
    ...
    fn lowerFunctions(
        ...
        stage: ?ast.Stage,
        stage_io_sets: ?*const [3][]const hir.Global,
    ) anyerror![]const hir.Function {
        var lowered = std.ArrayListUnmanaged(hir.Function){};
    ...
    fn lowerFunctionWithOptions(
        ...
        var lowered_statements = std.ArrayListUnmanaged(hir.Statement){};
        ...
        if (!return_type.isVoid() and lowered_body.len > 0 and lowered_body[lowered_body.len - 1].data == .expr) {
            var replaced = try self.allocator.dupe(hir.Statement, lowered_body);
    ...
    fn lowerStatementList(self: *Builder, statements: []const *ast.Stmt, context: *FunctionContext) anyerror![]const hir.Statement {
        var items = std.ArrayListUnmanaged(hir.Statement){};
        defer items.deinit(self.allocator);
    ...
    fn lowerStmtInto(
        self: *Builder,
        list: *std.ArrayListUnmanaged(hir.Statement),
        statement: *ast.Stmt,
    ...
    fn lowerSingleStatementSlice(self: *Builder, statement: *ast.Stmt, context: *FunctionContext) anyerror![]const hir.Statement {
        var nested = try context.clone(self.allocator);
        defer nested.deinit();
        var items = std.ArrayListUnmanaged(hir.Statement){};
        defer items.deinit(self.allocator);
    ...
    fn lowerIf(self: *Builder, if_stmt: ast.IfStmt, context: *FunctionContext, index: usize) anyerror!hir.Statement {
        ...
        const else_body = if (index + 1 < if_stmt.branches.len) blk: {
            ...
            const items = try self.allocator.alloc(hir.Statement, 1);
            items[0] = nested_if;
            break :blk items;
    ...
    fn lowerExpr(self: *Builder, expr: *ast.Expr, context: *FunctionContext) anyerror!*hir.Expr {
        return switch (expr.data) {
                ...
                if (member.target.data == .self_ref) {
                    if (context.self_type != null) {
                        ...
                        if (builtins.resolveMethod(member.name, self_type, &.{})) |_| {
                            const args = try self.allocator.alloc(*hir.Expr, 1);
                            args[0] = self_expr;
                            break :blk try self.makeExprAt(expr.position, try self.resolvedExprType(expr, context), .{
                                .call = .{
                                    .name = member.name,
            ...
            .call => |call| blk: {
                var args = std.ArrayListUnmanaged(*hir.Expr){};
    ...
    fn lowerMatchExpr(
        ...
        try self.lowered_globals.append(self.allocator, .{
            ...
            .params = try self.allocator.dupe(hir.Param, &.{
                .{
                    .name = "__match_value",
                    .ty = value_type,
                    .is_inout = false,
    ...
    fn lowerMatchSwitch(
        ...
        var cases = std.ArrayListUnmanaged(hir.SwitchCase){};
    ...
    fn lowerMatchArms(
        ...
        var then_body = std.ArrayListUnmanaged(hir.Statement){};
    ...
    fn lowerMatchCaseBody(
        ...
        const body = try self.allocator.alloc(hir.Statement, 1);
        body[0] = statement;
        return body;
    }

    fn matchDefaultBody(self: *Builder, result_type: types.Type) anyerror![]const hir.Statement {
    ...
    fn finalizeImplicitReturn(
        _: *Builder,
        body: *std.ArrayListUnmanaged(hir.Statement),
        result_type: types.Type,
        source_line: u32,
    ) anyerror!void {
    ...
    fn defaultValueExpr(self: *Builder, ty: types.Type) anyerror!*hir.Expr {
        return switch (ty) {
    ...
    fn makeExpr(self: *Builder, ty: types.Type, data: hir.Expr.Data) anyerror!*hir.Expr {
        return self.makeExprAt(null, ty, data);
    }

    fn makeExprAt(self: *Builder, position: ?ast.Position, ty: types.Type, data: hir.Expr.Data) anyerror!*hir.Expr {
        const expr = try self.allocator.create(hir.Expr);
        expr.* = .{
            .ty = ty,
    ...
    fn makeStatement(_: *Builder, source_line: ?u32, data: hir.Statement.Data) hir.Statement {
        return Builder.makeStatementWithColumn(source_line, null, data);
    }

    fn makeStatementAt(_: *Builder, position: ?ast.Position, data: hir.Statement.Data) hir.Statement {
    ...
    fn makeStatementWithColumn(source_line: ?u32, source_column: ?u32, data: hir.Statement.Data) hir.Statement {
        return .{
    ...
    fn lowerIo(_: *Builder, decl: ast.IoDecl) anyerror!hir.Global {
        return .{
            .name = decl.name,
