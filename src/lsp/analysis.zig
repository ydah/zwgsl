...
fn builtinItems() []const StaticDoc {
    return &.{
        ...
        .{ .name = "distance", .detail = "fn distance(a: Vec(N) | Sca, b: Vec(N) | Sca) -> Float", .documentation = "Returns the distance between matching scalar or vector values." },
        ...
        .{ .name = "refract", .detail = "fn refract(i: Vec(N), n: Vec(N), eta: Float) -> Vec(N)", .documentation = "Refracts an incident vector through a surface normal." },
        .{ .name = "mix", .detail = "fn mix(a: T, b: T, t: T | Sca) -> T", .documentation = "Linearly interpolates between values." },
        .{ .name = "clamp", .detail = "fn clamp(x: T, min: T | Sca, max: T | Sca) -> T", .documentation = "Clamps a numeric value between lower and upper bounds." },
        .{ .name = "min", .detail = "fn min(a: T, b: T | Sca) -> T", .documentation = "Returns the component-wise minimum." },
        .{ .name = "max", .detail = "fn max(a: T, b: T | Sca) -> T", .documentation = "Returns the component-wise maximum." },
        .{ .name = "mod", .detail = "fn mod(x: T, y: T | Sca) -> T", .documentation = "Returns the remainder of numeric values." },
        .{ .name = "pow", .detail = "fn pow(x: T, y: T | Sca) -> T", .documentation = "Raises numeric values to a power." },
        .{ .name = "step", .detail = "fn step(edge: T | Sca, x: T) -> T", .documentation = "Returns 0 or 1 based on an edge comparison." },
        .{ .name = "smoothstep", .detail = "fn smoothstep(edge0: T | Sca, edge1: T | Sca, x: T) -> T", .documentation = "Performs smooth Hermite interpolation between two edges." },
        .{ .name = "atan", .detail = "fn atan(y_over_x: T) -> T; fn atan(y: T, x: T) -> T", .documentation = "Computes the arctangent for one or two numeric arguments." },
        .{ .name = "abs", .detail = "fn abs(x: T) -> T", .documentation = "Returns the absolute value." },
        .{ .name = "sign", .detail = "fn sign(x: T) -> T", .documentation = "Returns the sign of a numeric value." },
        .{ .name = "floor", .detail = "fn floor(x: T) -> T", .documentation = "Rounds down to the nearest integer value." },
        .{ .name = "ceil", .detail = "fn ceil(x: T) -> T", .documentation = "Rounds up to the nearest integer value." },
        .{ .name = "fract", .detail = "fn fract(x: T) -> T", .documentation = "Returns the fractional part." },
        .{ .name = "sqrt", .detail = "fn sqrt(x: T) -> T", .documentation = "Returns the square root." },
        .{ .name = "exp", .detail = "fn exp(x: T) -> T", .documentation = "Returns e raised to the given value." },
        .{ .name = "log", .detail = "fn log(x: T) -> T", .documentation = "Returns the natural logarithm." },
        .{ .name = "sin", .detail = "fn sin(x: T) -> T", .documentation = "Returns the sine." },
        .{ .name = "cos", .detail = "fn cos(x: T) -> T", .documentation = "Returns the cosine." },
        .{ .name = "tan", .detail = "fn tan(x: T) -> T", .documentation = "Returns the tangent." },
        .{ .name = "asin", .detail = "fn asin(x: T) -> T", .documentation = "Returns the arcsine." },
        .{ .name = "acos", .detail = "fn acos(x: T) -> T", .documentation = "Returns the arccosine." },
...
fn numericMethodItems() []const CompletionItem {
    return &.{
        ...
        .{ .label = "mod", .kind = 2, .detail = "method" },
        .{ .label = "pow", .kind = 2, .detail = "method" },
        .{ .label = "step", .kind = 2, .detail = "method" },
        .{ .label = "smoothstep", .kind = 2, .detail = "method" },
        .{ .label = "atan", .kind = 2, .detail = "method" },
        .{ .label = "abs", .kind = 2, .detail = "method" },
        .{ .label = "sign", .kind = 2, .detail = "method" },
        .{ .label = "floor", .kind = 2, .detail = "method" },
        .{ .label = "ceil", .kind = 2, .detail = "method" },
        .{ .label = "fract", .kind = 2, .detail = "method" },
        .{ .label = "sqrt", .kind = 2, .detail = "method" },
        .{ .label = "exp", .kind = 2, .detail = "method" },
        .{ .label = "log", .kind = 2, .detail = "method" },
        .{ .label = "sin", .kind = 2, .detail = "method" },
        .{ .label = "cos", .kind = 2, .detail = "method" },
        .{ .label = "tan", .kind = 2, .detail = "method" },
        .{ .label = "asin", .kind = 2, .detail = "method" },
        .{ .label = "acos", .kind = 2, .detail = "method" },
